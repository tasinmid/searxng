...
def custom_api():
    """Custom API endpoint returning only search result source URLs for 'general' category.

    Supports parameters:
      q: search query
      rqs: number of URLs to return (e.g., 5 or 10)
    """
    # 1. API Key Authentication Check
    expected_key = os.environ.get('SEARXNG_API_KEY', 'ultrasecretapikey')

    # Check X-API-KEY header, query params 'key'/'api_key', or form params
    provided_key = (
        sxng_request.headers.get('X-API-KEY') or
        sxng_request.args.get('key') or
        sxng_request.args.get('api_key') or
        sxng_request.form.get('key') or
        sxng_request.form.get('api_key')
    )

    # Check Authorization: Bearer <key> header
    auth_header = sxng_request.headers.get('Authorization')
    ...
    # 2. Get query parameter 'q'
    q = sxng_request.args.get('q') or sxng_request.form.get('q')
    if not q:
        return Response(json.dumps({"error": "No query provided"}), status=400, mimetype='application/json')

    # 2. Get parameter 'rqs'
    rqs_val = sxng_request.args.get('rqs') or sxng_request.form.get('rqs')
    limit = None
    if rqs_val:
        try:
            limit = int(rqs_val)
            if limit < 1:
                return Response(json.dumps({"error": "rqs parameter must be a positive integer"}), status=400, mimetype='application/json')
        except ValueError:
            return Response(json.dumps({"error": "Invalid rqs parameter (must be an integer)"}), status=400, mimetype='application/json')

    # 3. Build form parameters for the search
    form_data = {
        'q': q,
        'categories': 'general'
    }
    ...
    try:
        ...
        # Filter engines to only keep 'general' category
        search_query.engineref_list = [
            engineref for engineref in search_query.engineref_list
            if engineref.category == 'general'
        ]
        ...
        logger.exception("Custom API search error")
        return Response(json.dumps({"error": "Search failed"}), status=500, mimetype='application/json')

    # 4. Extract URLs
    results = result_container.get_ordered_results()
    urls = []
    for r in results:
        # Avoid duplicates or invalid entries, only keep standard result URLs
        if 'url' in r and r['url']:
            urls.append(r['url'])

    # Apply rqs limit if provided
    if limit is not None:
        urls = urls[:limit]

    # Return only the source URLs as a JSON list
